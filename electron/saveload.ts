...
ipcMain.handle(LOAD_FILE, async (event, title: string, defaultPath: string) => {
  ...
  if (!dialogResult.canceled && dialogResult.filePaths.length > 0) {
    return await promises.readFile(dialogResult.filePaths[0])
  } else {
    throw new Error('User cancelled the dialog')
  }
})

ipcMain.handle(SAVE_FILE, async (event, title: string, defaultPath: string, data: string) => {
