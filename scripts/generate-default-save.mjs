...
const LfoShape = { Sin: 0, Ramp: 1, Square: 2, Saw: 3, Noise: 4 }
...
function mkSplit(baseParams, randomizer = defaultRandomizer, groups = {}) {
  return { baseParams, randomizer, groups }
}

/** Mirrors createBuiltinLayer defaults from BuiltinVisualizer.ts */
...
const lightSceneDefs = [
  {
    ...
    name: 'Warm Wash',
    epicness: 0.08,
    modulators: [mkLfo(LfoShape.Sin, 16, [{ brightness: 1 }])],
    splitScenes: [
      mkSplit({ hue: 0.08, saturation: 0.85, brightness: 0.55, x: 0.5, y: 0.5, width: 1, height: 1 }),
    ],
  },
  {
    id: 'ex_cool_wash',
    name: 'Cool Wash',
    epicness: 0.18,
    modulators: [mkLfo(LfoShape.Sin, 12, [{ brightness: 1 }])],
    splitScenes: [
      mkSplit({ hue: 0.58, saturation: 0.75, brightness: 0.5, x: 0.5, y: 0.5, width: 1, height: 1 }),
    ],
  },
  {
    id: 'ex_hue_sweep',
    name: 'Hue Sweep',
    epicness: 0.28,
    modulators: [mkLfo(LfoShape.Ramp, 8, [{ hue: 1 }])],
    splitScenes: [
      mkSplit({ hue: 0.5, saturation: 1, brightness: 0.85, x: 0.5, y: 0.5, width: 0.85, height: 1 }),
    ],
  },
  {
    id: 'ex_lr_chase',
    name: 'Left-Right Chase',
    epicness: 0.38,
    modulators: [mkLfo(LfoShape.Sin, 4, [{ x: 1, hue: 0.65 }])],
    splitScenes: [
      mkSplit({ hue: 0.72, saturation: 1, brightness: 0.9, x: 0.5, y: 0.5, width: 0.55, height: 1 }),
    ],
  },
  {
    id: 'ex_bright_pulse',
    ...
    epicness: 0.48,
    modulators: [mkLfo(LfoShape.Square, 2, [{ brightness: 1 }])],
    splitScenes: [
      mkSplit({ hue: 0.5, saturation: 0.9, brightness: 0.45, x: 0.5, y: 0.5, width: 0.9, height: 1 }),
    ],
  },
  {
    id: 'ex_sat_breathe',
    name: 'Saturation Breathe',
    epicness: 0.55,
    modulators: [mkLfo(LfoShape.Sin, 6, [{ saturation: 1 }])],
    splitScenes: [
      mkSplit({ hue: 0.42, saturation: 0.35, brightness: 0.8, x: 0.5, y: 0.5, width: 0.8, height: 1 }),
    ],
  },
  {
    id: 'ex_width_swell',
    name: 'Width Swell',
    epicness: 0.62,
    modulators: [mkLfo(LfoShape.Saw, 4, [{ width: 1, brightness: 0.75 }])],
    splitScenes: [
      mkSplit({ hue: 0.66, saturation: 1, brightness: 0.55, x: 0.5, y: 0.5, width: 0.35, height: 1 }),
    ],
  },
  {
    id: 'ex_dual_zone',
    name: 'Dual Color Zones',
    epicness: 0.72,
    modulators: [mkLfo(LfoShape.Sin, 4, [{ x: 1 }, { x: 0 }])],
    splitScenes: [
      mkSplit({ hue: 0, saturation: 1, brightness: 0.9, x: 0.35, y: 0.5, width: 0.42, height: 1 }),
      mkSplit({ hue: 0.66, saturation: 1, brightness: 0.9, x: 0.65, y: 0.5, width: 0.42, height: 1 }),
    ],
  },
  {
    id: 'ex_tri_fan',
    ...
    epicness: 0.8,
    modulators: [
      mkLfo(LfoShape.Sin, 4, [{ x: 1 }, {}, {}]),
      mkLfo(LfoShape.Sin, 4, [{}, { x: 1 }, {}], { phaseShift: 0.33 }),
      mkLfo(LfoShape.Sin, 4, [{}, {}, { x: 1 }], { phaseShift: 0.66 }),
    ],
    splitScenes: [
      mkSplit({ hue: 0, saturation: 1, brightness: 0.85, x: 0.25, y: 0.5, width: 0.3, height: 1 }),
      mkSplit({ hue: 0.33, saturation: 1, brightness: 0.85, x: 0.5, y: 0.5, width: 0.3, height: 1 }),
      mkSplit({ hue: 0.66, saturation: 1, brightness: 0.85, x: 0.75, y: 0.5, width: 0.3, height: 1 }),
    ],
  },
  {
    id: 'ex_random_spark',
    ...
    epicness: 0.88,
    modulators: [
      mkLfo(LfoShape.Noise, 4, [{ hue: 1 }]),
      mkLfo(LfoShape.Sin, 2, [{ saturation: 0.6 }]),
    ],
    splitScenes: [
      mkSplit(
        { hue: 0.5, saturation: 0.85, brightness: 0.9, randomize: 1 },
        { triggerPeriod: 0.5, triggerDensity: 0.35, envelopeRatio: 0.08, envelopeDuration: 0.75 }
      ),
    ],
  },
  {
    ...
    epicness: 0.94,
    modulators: [],
    splitScenes: [mkSplit({ hue: 0.5, saturation: 0, brightness: 1, strobe: 1 })],
  },
  {
    id: 'ex_strobe_color',
    name: 'Strobe Color Shift',
    epicness: 1,
    modulators: [mkLfo(LfoShape.Ramp, 2, [{ hue: 1 }])],
    splitScenes: [mkSplit({ hue: 0.5, saturation: 1, brightness: 1, strobe: 1 })],
  },
]
...
const lightIds = lightSceneDefs.map((s) => s.id)
const lightById = Object.fromEntries(
  lightSceneDefs.map((s) => [
    s.id,
