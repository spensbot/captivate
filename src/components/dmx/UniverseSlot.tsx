  ...
  return count == 1
    ? (
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.8rem'}}>
        <div>{start}</div><div>-</div><div>{end}</div>
      </div>
    )
}
...
function FixtureSlot({ fixture, index }: { fixture: Fixture, index: number }) {
  ...
  return (
    <div className={`${classes.root} ${classes.fixture} ${isSelected ? classes.selected : null}`} onClick={() => { dispatch(setSelectedFixture(index))} }>
      <ChannelSpan start={start} count={count}/>
      <div style={{ display: 'flex', alignItems: 'center'}}>
      ...
      {isSelected ? (
        <div style={{ display: 'flex', fontSize: '0.9rem', zIndex: 1 }}>
          <ToggleButton isEnabled={!!fixture.window.x} onClick={setWindowEnabled('x', !fixture.window.x)}>X</ToggleButton>
          <ToggleButton isEnabled={!!fixture.window.y} onClick={setWindowEnabled('y', !fixture.window.y)}>Y</ToggleButton>
        </div>
      ): (
        <div style={{ fontSize: '0.8rem', color: 'fff7' }}>{fixtureType.manufacturer}</div>
      )}
    </div>
  )
}
