...
export default function MyFixtureEditing({ id }: Props) {
  const fixtureType = useTypedSelector(
    (state) => state.dmx.fixtureTypesByID[id]
  )
  const dispatch = useDispatch()
  const channelsCopy = fixtureType.channels.map((fixtureChannel) => ({
    ...fixtureChannel,
  }))
  ...
  function getChannelFields(fixtureChannel: FixtureChannel, index: number) {
    if (fixtureChannel.type === 'master') return null
    if (fixtureChannel.type === 'color')
      return (
        <>
          <Select
            labelId={index + 'color label'}
            id={index + 'color'}
            value={fixtureChannel.color}
  ...
  function getChannels() {
    return formik.values.channels.map((fixtureChannel, index) => {
      return (
        ...
        >
          <span style={{ fontSize: '1.3rem', margin: '0 1rem 0 0' }}>
            {index + 1}
          </span>
          <Select
            labelId={'channel' + index + 'type label'}
            ...
            >
              ...
              <IconButton
                ...
                onClick={() => {
                  formik.setValues({
                    ...formik.values,
                    channels: [...formik.values.channels, { type: 'master' }],
                  })
                }}
              >
                <AddIcon />
