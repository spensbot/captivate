...
export default function Groups({}: Props) {
  ...
  const items = groups.map((group, i) => {
    ...
    if (activeFixtureGroups !== null) {
      const match = activeFixtureGroups.findIndex(
        (fixtureGroup) => fixtureGroup === group
      )
      ...
      const toggleOnClick = () => {
        ...
        toggleOn
          ? activeFixtureGroupsCopy.splice(match, 1)
          : activeFixtureGroupsCopy.push(group)
  ...
  return (
    <Root>
      <Header>
        ...
        <Spacer />
        <IconButton
          onClick={() => dispatch(setGroups([...groups, 'New Group']))}
        >
          <AddIcon />
        </IconButton>
      </Header>
