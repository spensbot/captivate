...
export enum Deprecated_LfoShape {
  Sin,
  Ramp,
}

export interface Deprectated_Lfo extends Lfo {
  shape: Deprecated_LfoShape
}
...
export function fixLightScenes(light: LightScenes_t) {
  ...
  for (const modulator of modulators(light)) {
    const deprecated_modulator = modulator as Deprecated_Modulator
    if (deprecated_modulator.modulation !== undefined) {
      modulator.splitModulations.unshift(deprecated_modulator.modulation)
      delete deprecated_modulator.modulation
    }
    if (deprecated_modulator.mod === undefined) {
      const deprecated_lfo = deprecated_modulator.lfo
      if (deprecated_lfo !== undefined) {
        modulator.mod = {
          ...deprecated_lfo,
          type:
            deprecated_lfo.shape === Deprecated_LfoShape.Sin ? 'Sin' : 'Ramp',
        }
        delete deprecated_modulator.lfo
      }
    }
  }
...
export function fixDeviceState(deviceState: DeviceState) {
  if (deviceState.connectable.audio === undefined) {
    deviceState.connectable.audio = []
  }
}
